...
import { ArrowLeft, Camera, ExternalLink, FilePlus2, Loader2, MapPin, Plus, Sparkles, Trash2 } from 'lucide-react';
...
import { CONTRIBUTION_TYPES, type Contribution, type ContributionType, type PlaceCategory } from '../types';
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (canCreatePlace && createPlaceProfile) {
        ...
        placeId = place.id;
        setLastCreatedPlaceId(place.id);
      }
      const contribution = await createContribution({ authorId: user.uid, type, title, description, location, city, country: userProfile?.currentCountry || 'Brazil', links: link.trim() ? [link.trim()] : [], metadata: metadata.trim() ? { details: metadata.trim() } : {}, placeId });
      setContributions((current) => [contribution, ...current]);
      setTitle(''); setDescription(''); setLocation(''); setLink(''); setMetadata('');
    } catch (publishError) {
