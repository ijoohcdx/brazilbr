...
import { ArrowLeft, Compass, FileText, Loader2, MapPinned, Search, UsersRound } from 'lucide-react';
...
const resultIcon = (type: SearchResult['type']) => type === 'person' ? UsersRound : type === 'place' ? MapPinned : type === 'post' ? FileText : Compass;

export const SearchScreen: React.FC<SearchScreenProps> = ({ onBack, onOpenProfile, onOpenPlace }) => {
  const { user } = useAuth();
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try { setResults(await searchAcrossBrazilBR(term, user.uid)); } catch (searchError) { console.error('Could not search BrazilBR:', searchError); setError('Search is unavailable right now. Please try again.'); setResults([]); } finally { setLoading(false); }
  };

  const openResult = (result: SearchResult) => {
    if (result.type === 'person') onOpenProfile(result.id);
    if (result.type === 'place') onOpenPlace(result.id);
    if (result.type === 'contribution' && result.placeId) onOpenPlace(result.placeId);
  };
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-md bg-stone-50 px-5 py-7 sm:px-8">
      ...
      {results.length > 0 && <div className="mt-6 space-y-3">{results.map((result) => { const Icon = resultIcon(result.type); const actionable = result.type === 'person' || result.type === 'place' || Boolean(result.placeId); return <button key={`${result.type}-${result.id}`} type="button" onClick={() => openResult(result)} disabled={!actionable} className={`w-full rounded-3xl border border-stone-200 bg-white p-4 text-left shadow-sm ${actionable ? 'transition hover:border-emerald-300' : 'cursor-default opacity-90'}`}><div className="flex gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700"><Icon className="h-5 w-5" /></div><div className="min-w-0"><div className="flex items-center gap-2"><h2 className="truncate font-bold text-stone-900">{result.title}</h2><span className="shrink-0 rounded-full bg-stone-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-stone-500">{result.type}</span></div><p className="mt-1 text-xs font-semibold capitalize text-emerald-700">{result.subtitle}</p>{result.description && <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-stone-600">{result.description}</p>}{actionable && <p className="mt-2 text-[11px] font-bold text-emerald-700">Open details</p>}</div></div></button>; })}</div>}
    </div>
  );
};
