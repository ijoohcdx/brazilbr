...
import { Compass, HelpCircle, Loader2, LogOut, MapPin, MessageCircle, User as UserIcon } from 'lucide-react';
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col bg-gradient-to-b from-emerald-50/50 via-stone-50 to-stone-100 px-5 py-7 sm:px-8">
      <header className="flex items-center justify-between"><div className="flex items-center gap-2.5"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-sm font-bold text-white shadow-sm">BR</div><div><span className="text-base font-bold uppercase tracking-tight text-stone-900">BrazilBR</span><p className="mt-0.5 text-[11px] font-medium leading-none text-stone-500">Your Nomadic Friend</p></div></div><button type="button" onClick={() => void logout()} disabled={isLoggingOut} className="flex items-center gap-1.5 rounded-full border border-stone-200/80 bg-white/80 px-3 py-1.5 text-xs font-semibold text-stone-500 transition hover:text-rose-600 disabled:opacity-50">{isLoggingOut ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <LogOut className="h-3.5 w-3.5" />}Sign out</button></header>
