...
import { ArrowLeft, Check, Flag, Globe2, Link2, Loader2, MapPin, ShieldAlert, UserRound, X } from 'lucide-react';
...
import { blockUser, reportUser } from '../firebase/safety';
...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({ uid, onBack, onOpenConversation }) => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const [publicProfile, currentConnection] = await Promise.all([getDiscoverableProfile(uid), getConnection(user.uid, uid)]);
  ...
  const block = async () => {
    ...
    try {
      ...
      setSafetyMessage('This person is blocked.');
      setConnection(null);
    } catch (blockError) {
      console.error('Could not block user:', blockError);
      setError('We could not block this person right now.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const submitReport = async (event: React.FormEvent) => {
    ...
    setError(null);
    try {
      await reportUser(user.uid, uid, reportReason);
      setReportReason('');
      setReportOpen(false);
      setSafetyMessage('Thanks. Your report was submitted.');
    } catch (reportError) {
      console.error('Could not submit report:', reportError);
      setError('We could not submit this report right now.');
    } finally {
      setActionLoading(false);
    }
  };
